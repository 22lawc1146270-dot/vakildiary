package com.vakildiary.app.domain.usecase.pdf

import com.vakildiary.app.core.Result
import com.vakildiary.app.data.pdf.PdfExportManager
import com.vakildiary.app.domain.model.Case
import com.vakildiary.app.domain.model.Payment
import java.io.File
import javax.inject.Inject

class ExportFeeLedgerUseCase @Inject constructor(
    private val pdfExportManager: PdfExportManager
) {
    suspend operator fun invoke(case: Case, payments: List<Payment>): Result<File> {
        return pdfExportManager.exportFeeLedger(case, payments)
    }
}
